...
#endif
#define LCD_DC_HIGH()                    HAL_GPIO_WritePin(DC_GPIO_Port,DC_Pin, GPIO_PIN_SET)
#define LCD_DC_LOW()                     HAL_GPIO_WritePin(DC_GPIO_Port,DC_Pin, GPIO_PIN_RESET)
...
/* Private functions ---------------------------------------------------------*/
static int8_t ST7789_send_cmd(uint8_t cmd);
static int8_t ST7789_send_data(void * data, uint16_t length);
...
int ST7789_init(void)
{
	...
	while ( init_cmds[cmd].databytes != 0xff )
	{
		...
		if((init_cmds[cmd].databytes&0x7F) != 0)
			ST7789_send_data( (uint8_t *)init_cmds[cmd].data, init_cmds[cmd].databytes&0x7F );
		if (init_cmds[cmd].databytes & 0x80) {
			My_mDelay(120);
		}
		cmd++;
	}
	ST7789_set_orientation(CONFIG_LCD_DISPLAY_ORIENTATION);
...
void ST7789_flush(uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end, uint16_t *color)
{
	...
	ST7789_send_data((uint8_t *)color,width*height*2);
}
void lcd_clear(uint16_t color)
{
	...
	lcd_address_set(0, 0, ST7789_WIDTH-1,ST7789_HEIGHT-1);
	LCD_DC_HIGH();	
	LCD_CS_LOW();
	buf = my_malloc(ST7789_WIDTH*2);
	if(buf)
	{
		...
		{
			disp_spi_send_data(buf,ST7789_WIDTH*2);
		}
		my_free(buf);
	}
	else
...
int32_t lcd_show_string(uint16_t x, uint16_t y, uint32_t size, char *str)
{
	...
	while (*p != '\0')
	{
		if (x > ST7789_WIDTH - size / 2)
		{
			x = 0;
			y += size;
		}
		if (y > ST7789_HEIGHT - size)
		{
			y = x = 0;
			lcd_clear(RED);
		}
...
{         
	...
	{ 
		...
		{   
			if(x>(ST7789_WIDTH-size)||y>(ST7789_HEIGHT-size)) 
				return;  
			bHz=0;//有汉字库    
...
{
#if (LCD_ST7789 == 1)
	uint8_t data[4] = {0x70, 0xA0, 0x00, 0xC0};
#else
