 ...
 */
__weak int32_t PI_Controller( PID_Handle_t * pHandle, int32_t wProcessVarError )
{
	...
	/* 比例项计算*/
    pHandle->wProportionalTerm = pHandle->hKpGain * wProcessVarError;
	/* 积分项计算 */
	...
	wOutput_32 = ( pHandle->wProportionalTerm >> pHandle->hKpDivisorPOW2 ) + ( pHandle->wIntegralTerm >> pHandle->hKiDivisorPOW2 );
#endif
	if ( wOutput_32 > pHandle->hUpperOutputLimit )
    {
        wDischarge = pHandle->hUpperOutputLimit - wOutput_32;
        wOutput_32 = pHandle->hUpperOutputLimit;
    }
    else if ( wOutput_32 < pHandle->hLowerOutputLimit )
    {
        wDischarge = pHandle->hLowerOutputLimit - wOutput_32;
        wOutput_32 = pHandle->hLowerOutputLimit;
    }
    else { /* Nothing to do here */ }
	if ( pHandle->hKiGain != 0 )
	    pHandle->wIntegralTerm += wDischarge;
    
    return ( ( int32_t )( wOutput_32 ) );
}
#if defined (CCMRAM)
#if defined (__ICCARM__)
#pragma location = ".ccmram"
...
__weak int32_t PID_Controller( PID_Handle_t * pHandle, int32_t wProcessVarError )
{
	int32_t wDeltaError;
	...
	if ( pHandle->hKdGain != 0 ) /* derivative terms not used */
	{
		...
		pHandle->wDifferentialTerm = pHandle->hKdGain * wDeltaError;
        pHandle->wPrevProcessVarError = wProcessVarError;
		
		#ifdef FULL_MISRA_C_COMPLIANCY
		wTemp_output = PI_Controller( pHandle, wProcessVarError ) + (pHandle->wDifferentialTerm/(int32_t)pHandle->hKdDivisor);
		#else
		/* WARNING: the below instruction is not MISRA compliant, user should verify
		that Cortex-M3 assembly instruction ASR (arithmetic shift right)
		is used by the compiler to perform the shifts (instead of LSR
		logical shift right)*/
		wTemp_output = PI_Controller( pHandle, wProcessVarError ) + (pHandle->wDifferentialTerm>>pHandle->hKdDivisorPOW2);
		#endif
